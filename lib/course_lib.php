...
*/
class block_ckc_requests_manager_new_course extends stdClass {
 ...
 *
 */
function block_ckc_requests_manager_create_new_course_by_record_id($mid, $sendMail) {
    ...
    $new_course = new block_ckc_requests_manager_new_course();


    // Starting course creation process
    // Step 1/5
    $event = \block_ckc_requests_manager\event\course_process::create(array(
    'objectid' => '',
    'other' => '-'. get_string('stepnumber', 'block_ckc_requests_manager'). ' 1/5-' . get_string('startingcoursecreation','block_ckc_requests_manager'),
    'context' => $context,
    ));
    $event->trigger();
    ...
    $timestamp_startdate = $GLOBALS['DB']->get_field('block_ckc_requests_manager_config', 'value', array('varname'=>'startdate'), IGNORE_MULTIPLE);
    ...
    $mode = $GLOBALS['DB']->get_field('block_ckc_requests_manager_config', 'value', array('varname'=>'page1_field3status'));

    // what naming mode is operating
    $naming = $GLOBALS['DB']->get_field('block_ckc_requests_manager_config', 'value', array('varname'=>'naming'), IGNORE_MULTIPLE);

    //what short naming format is operating
    $snaming = $GLOBALS['DB']->get_field('block_ckc_requests_manager_config', 'value', array('varname'=>'snaming'), IGNORE_MULTIPLE);

    //get the record for the request
    $rec =  $GLOBALS['DB']->get_record('block_ckc_requests_manager_records', array('id'=>$mid));

    // Build up a course record based on the request.

    if(empty($rec->cate)){
		$new_course->category = $GLOBALS['CFG']->defaultrequestcategory;
	} else {
    ...
    $editoroptions = array('maxfiles' => EDITOR_UNLIMITED_FILES, 'maxbytes'=>$GLOBALS['CFG']->maxbytes, 'trusttext'=>false, 'noclean'=>true);
   ...
   //
    if (!empty($GLOBALS['CFG']->creatornewroleid)) {
       // deal with course creators - enrol them internally with default role
       $status = enrol_try_internal_enrol($course->id, $rec->createdbyid, $GLOBALS['CFG']->creatornewroleid);

       if(!$status){
           $event = \block_ckc_requests_manager\event\course_process::create(array(
            'objectid' => $objid,
            'other' => '-'. get_string('stepnumber', 'block_ckc_requests_manager').  '2/5- ' . get_string('failedtoenrolcreator','block_ckc_requests_manager'),
            'context' => $context,
            ));
            $event->trigger();
       }else {
            $event = \block_ckc_requests_manager\event\course_process::create(array(
            'objectid' => $objid,
            'other' => '-'. get_string('stepnumber', 'block_ckc_requests_manager'). ' 2/5: -' . get_string('enrolledcrator','block_ckc_requests_manager'),
            'context' => $context,
            ));
            $event->trigger();
       }
	...
	if ($autoKey == 0 || $autoKey == 1){
        ...
        if ($GLOBALS['CFG']->version >= 2013051400) {
        ...
        $enrolRes = $GLOBALS['DB']->insert_record('enrol', $enrollmentRecord);
        //
        ...
        if(!$enrolRes){
               ...
               global $context;
                ...
                'objectid' => $objid,
                'context' => $context,
                'other' => '-'. get_string('stepnumber', 'block_ckc_requests_manager'). ' 3/5 - ' . get_string('keyaddsuccess', 'block_ckc_requests_manager'),
                ));
                $event->trigger();
              ...
              global $context;
                ...
                'objectid' => $objid,
                'context' => $context,
                'other' => '-'. get_string('stepnumber', 'block_ckc_requests_manager'). '- 3/5 - '. get_string('keyaddfail', 'block_ckc_requests_manager'),
                ));
                $event->trigger();
	...
	if ($sendMail == true) {
      block_ckc_requests_manager_send_emails($course->id, $new_course->shortname, $new_course->fullname, $modkey, $mid);
	}
    ...
    'objectid' => $objid,
    'context' => $context,
    'other' => '-'. get_string('stepnumber', 'block_ckc_requests_manager').' 4/5 ' . get_string('updatingrecstatus','block_ckc_requests_manager'),
    ));
    $event->trigger();
	...
	$GLOBALS['DB']->update_record('block_ckc_requests_manager_records', $updatedRecord);
    ...
    // Step 5/5
    $event = \block_ckc_requests_manager\event\course_created::create(array(
    'objectid' => $objid,
    'context' => $context,
    'other' => '-'. get_string('stepnumber', 'block_ckc_requests_manager').' 5/5 course ID ' . $course->id . '',
    ));
    $event->trigger();
 ...
 *
 */
function block_ckc_requests_manager_send_emails($courseid, $modcode, $modname, $modkey, $mid){
	...
	global $GLOBALS['USER'], $GLOBALS['CFG'], $GLOBALS['DB'];
		...
		$currentRecord =  $GLOBALS['DB']->get_record('block_ckc_requests_manager_records', array('id'=>$mid));
		$GLOBALS['USER']_ids = '';
		$GLOBALS['USER']_ids = $currentRecord->createdbyid; // Add the current user

		$replaceValues = array();
	    $replaceValues['[course_code'] = $modcode;
	    $replaceValues['[course_name]'] = $modname;
	    $replaceValues['[e_key]'] = $modkey;
	    $replaceValues['[full_link]'] = $GLOBALS['CFG']->wwwroot .'/course/view.php?id=' . $courseid;
	    $replaceValues['[loc]'] = 'Location: ' . '';
	    $replaceValues['[req_link]'] = $GLOBALS['CFG']->wwwroot .'/blocks/ckc_requests_manager/view_summary.php?id=' . $courseid;

		//mail the user
		block_ckc_requests_manager_new_course_approved_mail_user($GLOBALS['USER']_ids, $replaceValues);
		//mail the admin
		block_ckc_requests_manager_new_course_approved_mail_admin($replaceValues);
