...
if (has_capability('block/cmanager:approverecord', $context)) {
    ...
    print_error(get_string('cannotapproverecords', 'block_ckc_requests_manager'));
}
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_ckc_requests_manager_approve_module_form extends moodleform
{
    ...
    function definition()
    {
        ...
        $mform->addElement('header', 'mainheader', get_string('approverequest_Title', 'block_ckc_requests_manager'));
        ...
        if ($type == '1') {
            ...
            $htmlOutput .= get_string('approverequest_New', 'block_ckc_requests_manager');
        }

        if ($type == '2') {
               $htmlOutput .= get_string('approverequest_Process', 'block_ckc_requests_manager');
        }
        ...
        $mform->addElement(
            'html',
            '<p><center><div name="addedmodules" id="addedmodules" align="left" style="border: 1px grey solid; width:700px;">
  ...
  // Close the class
$mform = new block_ckc_requests_manager_approve_module_form();
...
if ($mform->is_cancelled()) {
    ...
    print_header_simple(
        $streditinga,
        '',
        "<a href=\"$GLOBALS['CFG']->wwwroot/mod/$module->name/index.php?id=$course->id\">$strmodulenameplural</a> ->
     $strnav $streditinga",
        $mform->focus(),
        '',
        false
